# BSS-Certify v2.0 - Next Generation Skill Security Certification
...
| 特性 | v1.x | v2.0 |
|-----|------|------|
| 检测维度 | 4 维基础检查 | **6 维深度分析** |
| 威胁模式 | 15+ 基础模式 | **40+ 高级模式** |
| API 审计 | 基础识别 | **分类评级 + 风险评估** |
| 报告格式 | 基础 Markdown | **结构化 JSON + 增强 Markdown** |
| 团队支持 | 单人维护 | **多团队协作架构** |
| 检测精度 | 基础 | **熵值分析 + 上下文感知** |
...
1. **通过 cocoloop 安装** (推荐)
```bash
cocoloop install bss-certify-v2
```

2. **手动安装**
```bash
# 复制到 skill 目录
cp -r bss-certify-v2 ~/.claude/skills/
...
# 打包 skill
cd bss-certify-v2
zip -r bss-certify-v2.skill .

# 安装
claude skills install bss-certify-v2.skill
```
...
**BSS-Certify v2.0** - 让 Skill 安全检测进入 next level 🚀
